...
<!-- -------------------Search Result Start-------------------------- -->
                ...
                <button id="diffFileContent" type="button" class="btn btn-info">Diff</button>
...
<!-- -------------------Search Result End-------------------------- -->
	...
	/* get storageList */
	    ...
	    phonecatApp.controller('ListCtrl', function ($scope,$http) {	    	  
    	  ...
    	  alert(JSON.stringify(tmpProInfo));
    	  $http.post('../../loadParentFolders',JSON.stringify(tmpProInfo)).
    	  then(function(response) {
    	    // this callback will be called asynchronously
    	    // when the response is available
	        ...
	        $scope.selectFolder = function(folder){
	    		if(folder.id==projectInfo.projectCode){
	    			alert("ha....projectId\n\n\n"+$scope.folders[0].id);
	    			projectInfo.folderCodes.splice(1, projectInfo.folderCodes.length-1);
	    			projectInfo.folderName=null;
	    			projectInfo.fileCode=null;
	    			projectInfo.fileName=null;
	    		}else{
	    			for(var i=0;i<projectInfo.folderCodes.length;i++){
	    				if(projectInfo.folderCodes[i]==folder.id){
	    					projectInfo.folderCodes.splice(i, projectInfo.folderCodes.length-1-i);
	    	    			projectInfo.folderName=folder.name;
	    					break;
	    				}
	    			}
	    		}
	...
	/* when STORAGE button is clicked end */
		...
		function saveFile(){
			...
			tmpProInfo.fileCode  = projectInfo.fileCode;
			tmpProInfo.fileContent  = editor.getValue();
        	...
        	$.ajax({ 
        		type : 'POST',
       			data : JSON.stringify(tmpProInfo),
...
/* ------------- save File fucntion start --------------------- */
		function updateFile(){
            ...
            alert("updateFile   >>>   updateFileContent  \n"+JSON.stringify(tmpProInfo));
            /* ajax start */
        	$.ajax({ 
        		type : 'POST',
       			data : JSON.stringify(tmpProInfo),
...
/* ------------- save Commit File fucntion start --------------------- */
		function updateFileCommit(){
			...
			alert("updateFileContentWithCommit  \n"+JSON.stringify(tmpProInfo));
			tmpProInfo.fileContent  = editor.getValue();
			tmpProInfo.commitTitle = $("#commitTitle").val();
			tmpProInfo.commitContent  = $("#commitContent").val();
            
            alert("updateFileCommit  >><<<updateFileContentWithCommit  \n"+JSON.stringify(tmpProInfo));
        	...
        	$.ajax({ 
        		type : 'POST',
       			...
       			success : function(a){
       				alert("여기는왜//.....제이슨이 오긴옴");
       				if(a.answer=="success"){
						alert("diff commit success...");
       					if(tmpProInfo.storageCode==tmpProInfo.memberCode){
       						window.location.href = "http://localhost:8089/socialPro/pages/realPage/project.html";	
       					}else{
       						window.location.href = "http://localhost:8089/socialPro/pages/realPage/teamProject.html";
       					}
	       				// back to welcomePage http://localhost:8089/socialPro/pages/realPage/memberWelcom.jsp
...
/* ------------- diff File fucntion start --------------------- */
		function searchDiffFile(){
			//send folderCode, name
            ...
            alert("searchDiffFile  ////////   searchFileDiff  \n"+JSON.stringify(tmpProInfo));
...
/* >>>>>>>>>>>>>>>>>>>>>>>  Window Load Start <<<<<<<<<<<<<<<<<<<<<<<<<<<< */	
	...
	/* set FileName */
			...
			$("#diffFileContent").bind("click",function(){searchDiffFile();});
